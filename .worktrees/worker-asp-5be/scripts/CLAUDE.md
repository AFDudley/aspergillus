...
- `check_stringly_dispatch.py` — ASP-FSM-STRINGLY (asp-5be): warns when an
  if/elif or `match` dispatches on `==` string-literal comparisons that
  shadow a same-module `Enum`'s values, so the dispatch bypasses
  exhaustiveness checking. Silenced by a `# asp-fsm: boundary-parse` marker
  comment anywhere in the flagged function's body (for
  serialization-boundary parsers), and silent when no same-module Enum
  exists to shadow.
